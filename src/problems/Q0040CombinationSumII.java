...
public class Q0040CombinationSumII {
    public List<List<Integer>> combinationSum2(int[] candidates, int target) {
        ...
        for (int i = 0; i < candidates.length; i++) {
            ...
            helper(new ArrayList<>(List.of(candidates[i])), candidates[i], result, candidates, target, i + 1);
            prev = candidates[i];
        }
        return result;
    }
    private void helper(List<Integer> current, int sum, List<List<Integer>> result, int[] candidates, int target, int minIndex) {
