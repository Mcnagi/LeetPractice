...
public class Q0026Test {
    ...
    public void test(int expectedK, int[] expectedArray, int[] input) {
        ...
        Assertions.assertEquals(expectedK, q.removeDuplicates(),
                String.format("Expect to have %s with input: %s", expectedK, input));

        Assertions.assertTrue(arrayEqual(expectedArray, q.array),
                String.format("Expect to get: %s, but got: %s",
                        Arrays.toString(expectedArray),
                        Arrays.toString(q.array)));
    }
